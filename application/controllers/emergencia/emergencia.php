...
//            $this->carregarView($data);
    ...
    function novaevolucao($rae_id) {

        $data['data'] = date('d:m:Y');
        $data['hora'] = date('H:i');
        $data['ficha'] = $rae_id;
        $data['pesotemperatura'] = $this->emergencia_m->listarpesotemperatura($rae_id);
        //$this->carregarView($data,'emergencia/evolucao-ficha');
        $this->loadView('emergencia/evolucao-ficha', $data);
    }
    
    function excluirevolucao($evolucao_id, $ficha) {
        $this->emergencia_m->excluirevolucao($evolucao_id);
        $data['ficha'] = $ficha;
        $this->pesquisarevolucao($ficha);
    }

    function evolucao($emergencia_rae_id) {
        $data['impressao'] = $this->emergencia_m->evolucao($emergencia_rae_id);
        $data['empresa'] = $this->emergencia_m->empresa();
        $this->load->View('emergencia/emergencia-imprimirrae', $data);
    }
    ...
    function relatorioevolucao($evolucao_id) {
        $data['impressao'] = $this->emergencia_m->relatorioEvolucao($evolucao_id);
        $data['empresa'] = $this->emergencia_m->empresa();
...
//            $this->loadView('emergencia/emergencia-lista-solicitacaotomografia', $data);
    ...
    function gravarevolucao($ficha_id) {
        if($this->emergencia_m->gravarEvolucao($ficha_id)){
        $data['mensagem'] = "Evolucao efetuada com sucesso.";
        } else {
            $data['mensagem'] = "Erro ao gravar evolucao.";
        }
        $this->session->set_flashdata('message', $data['mensagem']);
